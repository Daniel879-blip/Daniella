...
def _init_conversations():
    if "conversations" not in st.session_state:
        st.session_state.conversations = {
            "New Chat": [{"role":"system","content":DEFAULT_SYSTEM_PROMPT}]
        }
    if "active_convo" not in st.session_state:
        st.session_state.active_convo = "New Chat"

def _load_active_messages():
    st.session_state.messages = st.session_state.conversations[st.session_state.active_convo]

def _save_active_messages():
    st.session_state.conversations[st.session_state.active_convo] = st.session_state.messages
...
.chat-title {font-weight:600; padding:6px 0;}
...
with st.sidebar:
    ...
    if st.button("➕ New Chat", use_container_width=True):
        ...
        st.session_state.conversations[name] = [{"role":"system","content":DEFAULT_SYSTEM_PROMPT}]
    ...
    for name in list(st.session_state.conversations.keys()):
        ...
        if cols[0].button(name, key=f"open_{name}", use_container_width=True):
            st.session_state.active_convo = name
            _load_active_messages()
            st.rerun()
        if cols[1].button("✕", key=f"del_{name}") and len(st.session_state.conversations)>1:
